...
function LastStep({ selectedfile, setSelectedfile, details, setDetails, setModal, goal }) {
  ...
  const CreatePlan = async () => {
    setIsloading(true);
    await Axios.post('/InvesteePlans/Create', {
      title: details?.title,
      unitAmount: goal && Math.floor(Number(goal) / 1000),
      unitCount: 10000,
      fundingType: details?.investmentPeriod?.key,
      investmentPeriod: details?.fundingType?.key,
      utilizationPlan: details?.utilizationPlan?.key,
      coverImagePaths: details?.coverImagePaths
    })
    ...
    <div className="w-full flex flex-col justify-between items-center gap-y-10  min-h-[400px] h-auto">
      <div className=" w-full flex flex-col items-center gap-y-5 ">
        ...
        <BoxUploadFile
          ...
          fileAddress={details?.coverImagePaths?.[0]}
          selectedfile={selectedfile}
          setFileAddress={(e) => setDetails((prev) => ({ ...prev, coverImagePaths: [e] }))}
      ...
      <button
        ...
        {isloading ? <BouncingDotsLoader /> : 'ثبت و ادامه'}
      </button>
    </div>
  );
}
